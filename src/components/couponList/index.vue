...
export default {
  ...
  mounted() {
            ...
            on: {
              "on-change": () => {
                ...
                if (this.productRow.id) {
                  this.$emit("nameId", this.productRow);
                  this.currentid = this.luckDraw ? this.currentid : 0;
                  this.isTemplate = this.luckDraw ? true : false;
                } else {
    ...
    if (this.many === "more" && !this.luckDraw) {
  ...
  methods: {
    ...
    tableList(type) {
      this.loading = true;
      type
        ? (this.tableFrom.receive_type = type)
        : (this.tableFrom.receive_type = 3);
      console.log("222");
      this.couponList = [];
      this.selectedIds = [];
      this.selectedNames = [];
